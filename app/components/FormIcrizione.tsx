...
export default function FormIscrizione() {
  ...
  const [formData, setFormData] = useState({
    nome: '', cognome: '', sesso: '', dataNascita: '', luogoNascita: '', provinciaNascita: '', cittadinanza: 'Italiana',
    codiceFiscale: '', indirizzoResidenza: '', cittaResidenza: '', capResidenza: '', provinciaResidenza: '',
    email: '', telefono: '', genitoreNome: '', genitoreCognome: '', genitoreContattoScelta: 'whatsapp', genitoreContatto: '',
    dataCertificato: '', fileCertificato: null as File | null, 
    consensoSalute: false, consensoRegolamento: false, consensoVideosorveglianza: false, consensoPrivacy: false,
  })
  ...
  const getErrors = () => {
    ...
    if (formData.codiceFiscale && !cfRegex.test(formData.codiceFiscale)) errs.codiceFiscale = "Codice errato";
    ...
    if (isMinorenne()) {
      ...
      else if (formData.genitoreContattoScelta === 'email' && !/^\S+@\S+\.\S+$/.test(formData.genitoreContatto)) errs.genitoreContatto = "Email non valida";
    }
    return errs;
  };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (type === 'checkbox') return setFormData(prev => ({ ...prev, [name]: checked }));
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => setTouched(prev => ({ ...prev, [e.target.name]: true }));
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => e.target.files && setFormData(prev => ({ ...prev, fileCertificato: e.target.files![0] }));
  ...
  const prevStep = () => { window.scrollTo({ top: 0, behavior: 'smooth' }); setStep(prev => prev - 1); }
    ...
    setIsInviandoOtp(true); // Disabilitiamo il bottone mentre spedisce
    
    // Decidiamo a quale email mandare (al genitore se minorenne, all'atleta se maggiorenne)
    ...
    try {
      ...
      if (response.ok) {
        // Se il postino ha spedito l'email, salviamo il codice segreto "di nascosto" e andiamo avanti
      ...
      setIsInviandoOtp(false); // Riattiviamo il bottone
    }
  }
    ...
    try {
      ...
      if (formData.fileCertificato) {
        ...
        const { data, error } = await supabase.storage.from('certificati_medici').upload(fileName, formData.fileCertificato);
      ...
      const { data: socioData, error: socioError } = await supabase.from('soci').insert([{
        ...
        indirizzo: formData.indirizzoResidenza, cap: formData.capResidenza, citta: formData.cittaResidenza, provincia_residenza: formData.provinciaResidenza,
        telefono: formData.telefono, email: formData.email, minorenne: under18,
  ...
  const ErrorMsg = ({ name }: { name: string }) => (touched[name] && errors[name]) ? <p className="text-red-500 text-xs mt-1.5 font-medium pl-1">⚠️ {errors[name]}</p> : null;
  ...
  const renderProgressBar = () => (
    <div className="mb-12 px-2 sm:px-6 max-w-xl mx-auto">
      <div className="flex items-center justify-between relative">
        ...
        <div className="absolute left-0 top-1/2 transform -translate-y-1/2 h-1 bg-yellow-400 rounded-full z-0 transition-all duration-500 ease-in-out" style={{ width: `${((step - 1) / 2) * 100}%` }}></div>
        
        {/* Etichetta Step 2 modificata in Certificato */}
        {[ { num: 1, label: 'Anagrafica' }, { num: 2, label: 'Certificato' }, { num: 3, label: 'Firma' } ].map((s) => (
          <div key={s.num} className="relative z-10 flex flex-col items-center">
            <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center font-bold text-base sm:text-lg transition-all duration-300 border-2 ${
              step >= s.num ? 'bg-yellow-400 border-yellow-400 text-gray-900 shadow-md' : 'bg-white border-gray-300 text-gray-400'
            }`}>
              {step > s.num ? '✓' : s.num}
            </div>
            <span className={`absolute -bottom-8 text-xs font-bold uppercase tracking-wider whitespace-nowrap ${step >= s.num ? 'text-gray-800' : 'text-gray-400'}`}>{s.label}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    return (
      <div className="min-h-screen bg-[#FAFAFA] py-12 px-4 flex items-start justify-center">
        <div className="bg-white shadow-xl rounded-2xl p-10 text-center max-w-2xl w-full border-t-8 border-yellow-400">
          ...
          <button onClick={() => window.location.reload()} className="bg-yellow-400 text-gray-900 px-8 py-4 rounded-xl font-bold text-lg hover:bg-yellow-500 transition-all shadow-md hover:shadow-lg">
            Torna alla Home
          </button>
        </div>
      </div>
  ...
  return (
    // Sfondo chiaro, elegante e pulito
    <div className="min-h-screen w-full bg-[#FAFAFA] py-10 font-sans text-gray-800">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        {renderProgressBar()}
         ...
         {/* IL FORM: Bianco, ombra elegante e leggero bordo giallo superiore */}
        <div className="bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-3xl p-6 sm:p-10 border border-gray-100 border-t-8 border-t-yellow-400 mt-16 relative z-10">
          ...
          {step === 1 && (
            <div className="animate-fade-in">
              <div className="hidden">{(() => { var isStep1Valid = Object.keys(errors).length === 0; return ''; })()}</div>
              ...
              {/* === BLOCCO LUOGO DI NASCITA E CITTADINANZA (Aggiornato) === */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                ...
                {/* La Cittadinanza è ora allineata e strutturata come gli altri campi */}
                <div className="md:col-span-3">
                  ...
                  <input 
                    ...
                    onBlur={handleBlur} 
                    className={getInputClass('cittadinanza')} 
              ...
              {under18 && (
                <div className="p-6 bg-yellow-50/50 border border-yellow-200 rounded-2xl mb-8">
                  <h3 className="text-lg font-bold text-gray-900 mb-5 flex items-center tracking-tight">
                    <span className="mr-2">👨‍👩‍👧</span> Dati Genitore / Tutore
                  </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input 
                        ...
                        name="genitoreContatto" value={formData.genitoreContatto} onChange={handleChange} onBlur={handleBlur}
              ...
              <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-gray-100 mt-4 gap-4">
                ...
                <button 
                  ...
                  className={`px-10 py-4 rounded-xl font-bold transition-all w-full md:w-auto shadow-sm ${
                    Object.keys(errors).length === 0 ? 'bg-gray-900 text-white hover:bg-gray-800 hover:shadow-lg hover:-translate-y-0.5' : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'
                  }`}
                >
                  Vai al Certificato →
                </button>
            ...
            <div className="animate-fade-in">
              ...
              <div className="mb-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[ { id: '1', nome: 'Sala Pesi', descrizione: 'Accesso libero agli attrezzi' }, { id: '2', nome: 'Corso Total Body', descrizione: 'Lezioni di gruppo settimanali' } ].map((attivita) => (
              ...
              <div className="mb-10 bg-gray-50 p-6 md:p-8 rounded-2xl border border-gray-100 relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 ml-2">
                  <div>
                    ...
                    <input type="date" name="dataCertificato" value={formData.dataCertificato} onChange={handleChange} className="w-full p-3.5 border border-gray-300 rounded-xl bg-white shadow-sm focus:ring-2 focus:ring-yellow-200 focus:border-yellow-400 outline-none transition-all text-gray-700 hover:border-gray-400" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Carica Foto o PDF *</label>
                    <input type="file" accept="image/*,.pdf" onChange={handleFileChange} className="w-full p-2 border border-dashed border-gray-300 rounded-xl bg-white shadow-sm file:mr-4 file:py-2 file:px-5 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-gray-100 file:text-gray-900 hover:file:bg-gray-200 cursor-pointer transition-all text-gray-500 hover:border-gray-400" />
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col-reverse md:flex-row justify-between items-center pt-8 border-t border-gray-100 gap-4">
                ...
                <button 
                  onClick={nextStep} disabled={!formData.dataCertificato || !formData.fileCertificato}
          ...
          {step === 3 && (
            <div className="animate-fade-in">
              ...
              <div className="space-y-4 mb-10">
                ...
                <label className="flex items-start cursor-pointer group p-4 bg-gray-50 hover:bg-yellow-50/50 rounded-xl transition-colors border border-gray-200 hover:border-yellow-400 shadow-sm">
                  <input type="checkbox" name="consensoPrivacy" checked={formData.consensoPrivacy} onChange={handleChange} className="mt-0.5 h-5 w-5 accent-yellow-400 border-gray-300 rounded cursor-pointer" />
                  <span className="ml-3.5 text-sm text-gray-600 leading-relaxed font-medium">
                    <strong className="text-gray-900 font-bold block mb-0.5">Uso Immagini e <a href="/privacy.pdf" target="_blank" onClick={(e) => e.stopPropagation()} className="text-yellow-600 hover:text-yellow-700 hover:underline transition-all">Privacy</a> (Facoltativo)</strong> 
                    Acconsento alla pubblicazione per fini istituzionali di foto/video sulle bacheche e i canali social dell'Associazione.
                  </span>
                </label>
              </div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6 sm:p-8 text-center shadow-inner relative overflow-hidden">
                {!otpInviato ? (
                  <div>
                    <p className="text-gray-600 mb-6 font-medium"> Invieremo un codice di sicurezza gratuito per firmare all'indirizzo email:<br/>
                    ...
                    <button 
                        ...
                        disabled={!formData.consensoSalute || !formData.consensoRegolamento || !formData.consensoVideosorveglianza || isInviandoOtp} 
                    ...
                    <p className="text-xs text-gray-400 mt-4 font-medium">* Accetta i 3 consensi obbligatori per sbloccare il pulsante.</p>
                  </div>
                ) : (
                  <div className="animate-fade-in">
                    ...
                    <div className="max-w-[260px] mx-auto mb-8">
                      <input type="text" maxLength={6} value={codiceOtpInserito} onChange={(e) => setCodiceOtpInserito(e.target.value.replace(/\D/g, ''))} className="w-full text-center text-3xl tracking-[0.3em] pl-[0.3em] font-mono p-4 border-2 border-gray-300 rounded-xl focus:border-yellow-400 focus:ring-4 focus:ring-yellow-100 focus:outline-none bg-white transition-all font-bold text-gray-900 shadow-inner" placeholder="------" />
                    </div>
                    <button onClick={handleConfermaFirma} disabled={codiceOtpInserito.length !== 6 || isSubmitting} className="bg-gray-900 text-white px-10 py-4 rounded-xl font-bold text-lg hover:bg-gray-800 disabled:bg-gray-200 disabled:text-gray-400 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 disabled:shadow-none disabled:transform-none w-full md:w-auto">{isSubmitting ? 'Salvataggio in corso...' : 'Firma e Concludi ✓'}</button>
                  </div>
                )}
              </div>
