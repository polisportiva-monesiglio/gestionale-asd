...
/**
 * L'indirizzo a cui tornerà il link. Lo calcola il server e non il browser:
 ...
 * ma quella lista è l'ultima difesa, non la prima.
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    // di sapere se l'indirizzo è riconosciuto, di proposito: altrimenti chi
    ...
    if (riconosciuta) {
      // Chiave pubblica anche qui: signInWithOtp è un'operazione che il
      // browser potrebbe fare da solo. Quello che la chiave di servizio
      // aggiunge è il controllo di cui sopra, non il potere di spedire.
      //
      // Deve però essere il client con i cookie, non uno costruito al volo.
      // Il flusso PKCE deposita un verificatore che resta a chi ha iniziato
      // la richiesta, e il callback lo rilegge per scambiare il codice con la
      // sessione. Un client usa e getta lo genera e lo butta via: il link
      // arriva regolarmente, ma al ritorno non c'è più niente con cui
      // completare lo scambio, e il socio finisce sulla pagina che gli dice
      // che non è riconosciuto — mentre invece lo era.
      const conCookie = await createClient()

      const { error } = await conCookie.auth.signInWithOtp({
